...
public class VineScript : MonoBehaviour
{
    ...
    void SpawnBranch()
    {
        ...
        VineScript newChildVine =
            Instantiate(Seed.VinePrefab, EndPoint.position, transform.rotation);
        ...
        newChildVine
            ...
            this,
            MaxTreeHeight,
            ParentAccumulatedTreeHeight + targetLength);

        // FixedJoint newFixedJoint = newChildVine.gameObject.AddComponent<FixedJoint>();
        newChildVine.GetComponent<FixedJoint>().connectedBody =
    ...
    {
        if (
            other.collider.TryGetComponent<Cutter>(out Cutter cutter) &&
            cutter.isCutting
        )
        {
            OnCut();
